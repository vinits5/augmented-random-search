...
def ppo_update(model, optimizer, ppo_epochs, mini_batch_size, states, actions, log_probs, returns, advantages, writer, frame_idx,  clip_param=0.2):
	...
	for _ in range(batch_size // mini_batch_size):
		for state, action, old_log_probs, return_, advantage in ppo_iter(mini_batch_size, states, actions, log_probs, returns, advantages):
			...
			dist = torch.distributions.Categorical(probabilities.cpu())
			entropy = dist.entropy().mean()
			new_log_probs = dist.log_prob(action)

			ratio = ((new_log_probs - old_log_probs).exp()).to(advantage)
			# print("Advantage",advantage.type())
			...
			loss = 0.5 * critic_loss + actor_loss - 0.001 * entropy.to(advantage)
			# print(entropy.type())
