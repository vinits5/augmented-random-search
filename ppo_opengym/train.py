...
def train(args):
	...
	envs = [utils.make_env() for i in range(num_envs)]
	envs = SubprocVecEnv(envs)
	# envs = gym.make('LunarLander-v2')
	# pdb.set_trace()	# Debug
	# print(num_inputs)
	num_inputs = envs.observation_space.shape[0]
	num_outputs = envs.action_space.n 	#discrete action space
	...
	while frame_idx < max_frames and not early_stop:
		...
		while not done and total_steps<1000:
			print('Steps taken: {} & Epoch: {}\r'.format(total_steps, int(frame_idx/20)))
			...
			next_state, reward, done, _ = envs.step(action_)
			# print(reward)
